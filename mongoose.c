...
static size_t mg_dns_parse_name_depth(const uint8_t *s, size_t len, size_t ofs, char *to, size_t tolen, size_t j,
                                      int depth) {
    size_t i = 0;
    if (tolen > 0 && depth == 0)
        to[0] = '\0';
            ...
            for (d = (struct dns_data *)c->mgr->active_dns_requests; d != NULL; d = tmp) {
                ...
                if (d->c->is_resolving) {
                    if (dm.resolved) {
                        ...
                        MG_DEBUG(("%lu %s is %I", d->c->id, dm.name, d->c->rem.is_ip6 ? 16 : 4,
...
static void mg_putchar_iobuf_static(char ch, void *param) { mg_pfn_iobuf_private(ch, param, false); }
...
static size_t bcpy(void (*out)(char, void *), void *ptr, uint8_t *buf, size_t len) {
    size_t i, n = 0;
        ...
        if (fmt[i] == '%') {
            ...
            if (c == 'd' || c == 'u' || c == 'x' || c == 'X' || c == 'p' || c == 'g' || c == 'f') {
                ...
                if (len == 6) {
                    ...
                    n += mg_xprintf(out, param, "%x:%x:%x:%x:%x:%x:%x:%x", mg_htons(p[0]), mg_htons(p[1]),
                                    mg_htons(p[2]), mg_htons(p[3]), mg_htons(p[4]), mg_htons(p[5]), mg_htons(p[6]),
                                    mg_htons(p[7]));
                } else {
                    n += mg_xprintf(out, param, "%d.%d.%d.%d", (int)buf[0], (int)buf[1], (int)buf[2], (int)buf[3]);
                }
            } else if (c == 'A') {
                // Print hardware addresses (currently Ethernet MAC)
                uint8_t *buf = va_arg(*ap, uint8_t *); // Pointer to the hw address
                n += mg_xprintf(out, param, "%02x:%02x:%02x:%02x:%02x:%02x", (int)buf[0], (int)buf[1], (int)buf[2],
...
static size_t p_write(void *fp, const void *buf, size_t len) { return fwrite(buf, 1, len, (FILE *)fp); }
...
static bool isx(int c) { return (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'); }
...
static void mg_http_parse_headers(const char *s, const char *end, struct mg_http_header *h, int max_headers) {
    int i;
...
void mg_http_reply(struct mg_connection *c, int code, const char *headers, const char *fmt, ...) {
    ...
    va_end(ap);
    if (c->send.len > 15) {
        mg_snprintf((char *)&c->send.buf[len - 14], 11, "%010lu", (unsigned long)(c->send.len - len));
        c->is_resp = 0;
...
void mg_http_serve_file(struct mg_connection *c, struct mg_http_message *hm, const char *path,
    ...
    if (fd == NULL || fs->st(path, &size, &mtime) == 0) {
        ...
        // NOTE: mg_http_etag() call should go first!
               ...
               (inm = mg_http_get_header(hm, "If-None-Match")) != NULL && mg_vcasecmp(inm, etag) == 0) {
        mg_fs_close(fd);
        ...
        mg_printf(c,
                  ...
                  "Content-Length: %llu\r\n"
                  "%s%s%s\r\n",
                  status, mg_http_status_code_str(status), (int)mime.len, mime.ptr, etag, cl,
...
static void printdirentry(const char *name, void *userdata) {
    ...
    if (mg_snprintf(path, sizeof(path), "%s%c%s", d->dir, '/', name) > sizeof(path)) {
        ...
        mg_printf(d->c,
                  ...
                  n, path, slash, name, slash, (unsigned long)t, mod, flags & MG_FS_DIR ? (int64_t)-1 : (int64_t)size,
                  sz);
    }
}

static void listdir(struct mg_connection *c, struct mg_http_message *hm, const struct mg_http_serve_opts *opts,
                    char *dir) {
...
static int uri_to_path2(struct mg_connection *c, struct mg_http_message *hm, struct mg_fs *fs, struct mg_str url,
                        struct mg_str dir, char *path, size_t path_size) {
    int flags, tmp;
    ...
    if (flags == 0) {
        ...
        if (((mg_snprintf(path + n, path_size - n, "/" MG_HTTP_INDEX) > 0 && (tmp = fs->st(path, NULL, NULL)) != 0) ||
             (mg_snprintf(path + n, path_size - n, "/index.shtml") > 0 && (tmp = fs->st(path, NULL, NULL)) != 0))) {
            flags = tmp;
...
static int uri_to_path(struct mg_connection *c, struct mg_http_message *hm, const struct mg_http_serve_opts *opts,
...
static void deliver_chunked_chunks(struct mg_connection *c, size_t hlen, struct mg_http_message *hm, bool *next) {
    //  |  ... headers ... | HEXNUM\r\n ..data.. \r\n | ......
    //  +------------------+--------------------------+----
    //  |      hlen        |           chunk1         | ......
...
static void deliver_normal_chunks(struct mg_connection *c, size_t hlen, struct mg_http_message *hm, bool *next) {
...
static size_t roundup(size_t size, size_t align) { return align == 0 ? size : (size + align - 1) / align * align; }
...
char *mg_json_get_str(struct mg_str json, const char *path) {
    ...
    if (off >= 0 && len > 1 && json.ptr[off] == '"') {
        if ((result = (char *)calloc(1, (size_t)len)) != NULL &&
            !json_unescape(json.ptr + off + 1, (size_t)(len - 2), result, (size_t)len)) {
            free(result);
            result = NULL;
        }
    }
...
bool mg_log_prefix(int level, const char *file, int line, const char *fname) {
    if (level <= s_level) {
        ...
        if (p == NULL)
            p = strrchr(file, '\\');
        n = mg_snprintf(buf, sizeof(buf), "%llx %d %s:%d:%s", mg_millis(), level, p == NULL ? file : p + 1, line,
                        fname);
        if (n > sizeof(buf) - 2)
            n = sizeof(buf) - 2;
        while (n < sizeof(buf))
...
void mg_hexdump(const void *buf, size_t len) {
    ...
    for (i = 0; i < len; i++) {
        if ((i % 16) == 0) {
            ...
            logc(nibble((i >> 12) & 15)), logc(nibble((i >> 8) & 15)), logc(nibble((i >> 4) & 15)), logc('0'),
                logs("   ", 3);
        }
...
void mg_mqtt_login(struct mg_connection *c, const struct mg_mqtt_opts *opts) {
    ...
    if (opts->will_topic.len > 0 && opts->will_message.len > 0) {
        total_len += 4 + (uint32_t)opts->will_topic.len + (uint32_t)opts->will_message.len;
        hdr[7] |= MQTT_HAS_WILL;
    }
...
void mg_mqtt_pub(struct mg_connection *c, struct mg_str topic, struct mg_str data, int qos, bool retain) {
    ...
    MG_DEBUG(("%lu [%.*s] -> [%.*s]", c->id, (int)topic.len, (char *)topic.ptr, (int)data.len, (char *)data.ptr));
    if (qos > 0)
        len += 2;
    if (c->is_mqtt5)
        len++;
    ...
    if (ev == MG_EV_READ) {
        for (;;) {
            ...
            if (rc == MQTT_MALFORMED) {
                ...
                MG_VERBOSE(
                    ("%lu MQTT CMD %d len %d [%.*s]", c->id, mm.cmd, (int)mm.dgram.len, (int)mm.data.len, mm.data.ptr));
                switch (mm.cmd) {
                case MQTT_CMD_CONNACK:
                    ...
                    break;
                case MQTT_CMD_PUBLISH: {
                    MG_DEBUG(("%lu [%.*s] -> [%.*s]", c->id, (int)mm.topic.len, mm.topic.ptr, (int)mm.data.len,
                              mm.data.ptr));
                    if (mm.qos > 0) {
...
void mg_mqtt_disconnect(struct mg_connection *nc) { mg_mqtt_send_header(nc, MQTT_CMD_DISCONNECT, 0, 0); }

struct mg_connection *mg_mqtt_connect(struct mg_mgr *mgr, const char *url, const struct mg_mqtt_opts *opts,
...
void mg_rpc_vok(struct mg_rpc_req *r, const char *fmt, va_list *ap) {
    ...
    if (off > 0) {
        mg_xprintf(r->pfn, r->pfn_data, "{%Q:%.*s,%Q:", "id", len, &r->frame.ptr[off], "result");
...
void mg_rpc_verr(struct mg_rpc_req *r, int code, const char *fmt, va_list *ap) {
    ...
    mg_xprintf(r->pfn, r->pfn_data, "%Q:{%Q:%d,%Q:", "error", "code", code, "message");
    mg_vxprintf(r->pfn, r->pfn_data, fmt == NULL ? "null" : fmt, ap);
    mg_xprintf(r->pfn, r->pfn_data, "}}");
}
...
static size_t print_methods(mg_pfn_t pfn, void *pfn_data, va_list *ap) {
    ...
    for (h = *head; h != NULL; h = h->next) {
        ...
        len += mg_xprintf(pfn, pfn_data, "%s%.*Q", h == *head ? "" : ",", (int)h->method.len, h->method.ptr);
    }
    return len;
}
...
static void iolog(struct mg_connection *c, char *buf, long n, bool r) {
    if (n == MG_IO_WAIT) {
        ...
        if (c->is_hexdumping) {
            ...
            MG_INFO(("\n-- %lu %I %s %I %ld", c->id, 4, &usa.sin.sin_addr, r ? "<-" : "->", 4, &c->rem.ip, n));
...
bool mg_send(struct mg_connection *c, const void *buf, size_t len) {
    if (c->is_udp) {
        ...
        MG_DEBUG(("%lu %p %d:%d %ld err %d", c->id, c->fd, (int)c->send.len, (int)c->recv.len, n, MG_SOCKET_ERRNO));
...
bool mg_open_listener(struct mg_connection *c, const char *url) {
    ...
    if (!mg_aton(mg_url_host(url), &c->loc)) {
...
#if ((MG_ARCH == MG_ARCH_WIN32) || (MG_ARCH == MG_ARCH_UNIX) || (defined(LWIP_SOCKET) && SO_REUSE == 1))
        } else if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char *)&on, sizeof(on)) != 0) {
        ...
        } else if (setsockopt(fd, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (char *)&on, sizeof(on)) != 0) {
            // "Using SO_REUSEADDR and SO_EXCLUSIVEADDRUSE"
            MG_ERROR(("exclusiveaddruse: %d", MG_SOCKET_ERRNO));
#endif
        } else if (bind(fd, &usa.sa, slen) != 0) {
            MG_ERROR(("bind: %d", MG_SOCKET_ERRNO));
        } else if ((type == SOCK_STREAM && listen(fd, MG_SOCK_LISTEN_BACKLOG_SIZE) != 0)) {
...
static void read_conn(struct mg_connection *c) {
    ...
    if (c->recv.len >= MG_MAX_RECV_SIZE) {
        ...
        MG_DEBUG(("%lu %p snd %ld/%ld rcv %ld/%ld n=%ld err=%d", c->id, c->fd, (long)c->send.len, (long)c->send.size,
                  (long)c->recv.len, (long)c->recv.size, n, MG_SOCKET_ERRNO));
        iolog(c, buf, n, true);
    }
}
...
void mg_connect_resolved(struct mg_connection *c) {
    ...
    if (FD(c) == MG_INVALID_SOCKET) {
        ...
        if ((rc = connect(FD(c), &usa.sa, slen)) == 0) {
            ...
            MG_DEBUG(("%lu %p -> %I:%hu pend", c->id, c->fd, 4, &c->rem.ip, mg_ntohs(c->rem.port)));
            c->is_connecting = 1;
        } else {
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
    ...
    if (fd == MG_INVALID_SOCKET) {
...
#if (MG_ARCH != MG_ARCH_WIN32) && !MG_ENABLE_FREERTOS_TCP && (MG_ARCH != MG_ARCH_TIRTOS) && !MG_ENABLE_POLL
        ...
        MG_DEBUG(("%lu %p accepted %I.%hu -> %I.%hu", c->id, c->fd, 4, &c->rem.ip, mg_ntohs(c->rem.port), 4, &c->loc.ip,
                  mg_ntohs(c->loc.port)));
        mg_call(c, MG_EV_OPEN, NULL);
...
static bool mg_socketpair(MG_SOCKET_TYPE sp[2], union usa usa[2], bool udp) {
    ...
    *(uint32_t *)&usa->sin.sin_addr = mg_htonl(0x7f000001U); // 127.0.0.1
    ...
    if (udp && (sp[0] = socket(AF_INET, SOCK_DGRAM, 0)) != MG_INVALID_SOCKET &&
        (sp[1] = socket(AF_INET, SOCK_DGRAM, 0)) != MG_INVALID_SOCKET && bind(sp[0], &usa[0].sa, n) == 0 &&
        bind(sp[1], &usa[1].sa, n) == 0 && getsockname(sp[0], &usa[0].sa, &n) == 0 &&
        getsockname(sp[1], &usa[1].sa, &n) == 0 && connect(sp[0], &usa[1].sa, n) == 0 &&
        ...
        success = true;
               ...
               (sp[0] = socket(AF_INET, SOCK_STREAM, 0)) != MG_INVALID_SOCKET && connect(sp[0], &usa[0].sa, n) == 0 &&
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
    ...
    for (c = mgr->conns; c != NULL; c = tmp) {
        ...
        MG_VERBOSE(("%lu %c%c %c%c%c%c%c", c->id, c->is_readable ? 'r' : '-', c->is_writable ? 'w' : '-',
                    c->is_tls ? 'T' : 't', c->is_connecting ? 'C' : 'c', c->is_tls_hs ? 'H' : 'h',
...
static size_t mg_nce(const char *s, size_t n, size_t ofs, size_t *koff, size_t *klen, size_t *voff, size_t *vlen,
                     char delim) {
    size_t kvlen, kl;
...
bool mg_commalist(struct mg_str *s, struct mg_str *k, struct mg_str *v) { return mg_split(s, k, v, ','); }
...
void mg_timer_init(struct mg_timer **head, struct mg_timer *t, uint64_t ms, unsigned flags, void (*fn)(void *),
                   void *arg) {
...
#endif
    if ((rc = mbedtls_ssl_config_defaults(&tls->conf, c->is_client ? MBEDTLS_SSL_IS_CLIENT : MBEDTLS_SSL_IS_SERVER,
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
    ...
    MG_DEBUG(("%lu Setting TLS, CA: %s, cert: %s, key: %s", c->id, opts->ca == NULL ? "null" : opts->ca,
              opts->cert == NULL ? "null" : opts->cert, opts->certkey == NULL ? "null" : opts->certkey));
    tls->ctx = c->is_client ? SSL_CTX_new(SSLv23_client_method()) : SSL_CTX_new(SSLv23_server_method());
...
int mg_url_is_ssl(const char *url) {
    return strncmp(url, "wss:", 4) == 0 || strncmp(url, "https:", 6) == 0 || strncmp(url, "mqtts:", 6) == 0 ||
...
static void ws_handshake(struct mg_connection *c, const struct mg_str *wskey, const struct mg_str *wsproto,
...
struct mg_connection *mg_ws_connect(struct mg_mgr *mgr, const char *url, mg_event_handler_t fn, void *fn_data,
                                    const char *fmt, ...) {
    struct mg_connection *c = mg_connect(mgr, url, fn, fn_data);
    if (c != NULL) {
...
struct stm32_eth {
    volatile uint32_t MACCR, MACFFR, MACHTHR, MACHTLR, MACMIIAR, MACMIIDR, MACFCR, MACVLANTR, RESERVED0[2], MACRWUFFR,
        MACPMTCSR, RESERVED1, MACDBGR, MACSR, MACIMR, MACA0HR, MACA0LR, MACA1HR, MACA1LR, MACA2HR, MACA2LR, MACA3HR,
        MACA3LR, RESERVED2[40], MMCCR, MMCRIR, MMCTIR, MMCRIMR, MMCTIMR, RESERVED3[14], MMCTGFSCCR, MMCTGFMSCCR,
        RESERVED4[5], MMCTGFCR, RESERVED5[10], MMCRFCECR, MMCRFAECR, RESERVED6[10], MMCRGUFCR, RESERVED7[334], PTPTSCR,
        PTPSSIR, PTPTSHR, PTPTSLR, PTPTSHUR, PTPTSLUR, PTPTSAR, PTPTTHR, PTPTTLR, RESERVED8, PTPTSSR, PTPPPSCR,
        RESERVED9[564], DMABMR, DMATPDR, DMARPDR, DMARDLAR, DMATDLAR, DMASR, DMAOMR, DMAIER, DMAMFBOCR, DMARSWTR,
        RESERVED10[8], DMACHTDR, DMACHRDR, DMACHTBAR, DMACHRBAR;
};
#undef ETH
...
static bool mip_driver_stm32_init(struct mip_if *ifp) {
    ...
    ETH->MACA0LR =
        (uint32_t)(ifp->mac[3] << 24) | ((uint32_t)ifp->mac[2] << 16) | ((uint32_t)ifp->mac[1] << 8) | ifp->mac[0];
    if (ifp->queue.len == 0)
        ifp->queue.len = 8192;
    return true;
}
...
struct mip_driver mip_driver_stm32 = {mip_driver_stm32_init, mip_driver_stm32_tx, mip_driver_rx, mip_driver_stm32_up};
#endif
...
struct tm4c_emac {
    volatile uint32_t EMACCFG, EMACFRAMEFLTR, EMACHASHTBLH, EMACHASHTBLL, EMACMIIADDR, EMACMIIDATA, EMACFLOWCTL,
        EMACVLANTG, RESERVED0, EMACSTATUS, EMACRWUFF, EMACPMTCTLSTAT, RESERVED1[2], EMACRIS, EMACIM, EMACADDR0H,
        EMACADDR0L, EMACADDR1H, EMACADDR1L, EMACADDR2H, EMACADDR2L, EMACADDR3H, EMACADDR3L, RESERVED2[31], EMACWDOGTO,
        RESERVED3[8], EMACMMCCTRL, EMACMMCRXRIS, EMACMMCTXRIS, EMACMMCRXIM, EMACMMCTXIM, RESERVED4, EMACTXCNTGB,
        RESERVED5[12], EMACTXCNTSCOL, EMACTXCNTMCOL, RESERVED6[4], EMACTXOCTCNTG, RESERVED7[6], EMACRXCNTGB,
        RESERVED8[4], EMACRXCNTCRCERR, EMACRXCNTALGNERR, RESERVED9[10], EMACRXCNTGUNI, RESERVED10[239], EMACVLNINCREP,
        EMACVLANHASH, RESERVED11[93], EMACTIMSTCTRL, EMACSUBSECINC, EMACTIMSEC, EMACTIMNANO, EMACTIMSECU, EMACTIMNANOU,
        EMACTIMADD, EMACTARGSEC, EMACTARGNANO, EMACHWORDSEC, EMACTIMSTAT, EMACPPSCTRL, RESERVED12[12], EMACPPS0INTVL,
        EMACPPS0WIDTH, RESERVED13[294], EMACDMABUSMOD, EMACTXPOLLD, EMACRXPOLLD, EMACRXDLADDR, EMACTXDLADDR, EMACDMARIS,
        EMACDMAOPMODE, EMACDMAIM, EMACMFBOC, EMACRXINTWDT, RESERVED14[8], EMACHOSTXDESC, EMACHOSRXDESC, EMACHOSTXBA,
        EMACHOSRXBA, RESERVED15[218], EMACPP, EMACPC, EMACCC, RESERVED16, EMACEPHYRIS, EMACEPHYIM, EMACEPHYIMSC;
};
#undef EMAC
...
struct mip_driver mip_driver_tm4c = {mip_driver_tm4c_init, mip_driver_tm4c_tx, mip_driver_rx, mip_driver_tm4c_up};
#endif
...
static void w5500_txn(struct mip_spi *s, uint8_t block, uint16_t addr, bool wr, void *buf, size_t len) {
...
static void q_copyin(struct queue *q, const uint8_t *buf, size_t len, size_t head) {
...
static void mkpay(struct pkt *pkt, void *p) { pkt->pay = mkstr(p, (size_t)(&pkt->raw.ptr[pkt->raw.len] - (char *)p)); }
...
static struct ip *tx_ip(struct mip_if *ifp, uint8_t proto, uint32_t ip_src, uint32_t ip_dst, size_t plen) {
    struct eth *eth = (struct eth *)ifp->tx.ptr;
...
static void tx_udp(struct mip_if *ifp, uint32_t ip_src, uint16_t sport, uint32_t ip_dst, uint16_t dport,
...
static void tx_dhcp(struct mip_if *ifp, uint32_t src, uint32_t dst, uint8_t *opts, size_t optslen) {
...
static void rx_arp(struct mip_if *ifp, struct pkt *pkt) {
    if (pkt->arp->op == mg_htons(1) && pkt->arp->tpa == ifp->ip) {
        ...
        MG_DEBUG(("ARP op %d %I: %I?", mg_ntohs(pkt->arp->op), 4, &pkt->arp->spa, 4, &pkt->arp->tpa));
        struct eth *eth = (struct eth *)ifp->tx.ptr;
        struct arp *arp = (struct arp *)(eth + 1);
...
static void rx_icmp(struct mip_if *ifp, struct pkt *pkt) {
    ...
    if (pkt->icmp->type == 8 && pkt->ip != NULL && pkt->ip->dst == ifp->ip) {
        ...
        if (plen > space)
            plen = space;
        struct ip *ip = tx_ip(ifp, 1, ifp->ip, pkt->ip->src, sizeof(struct icmp) + plen);
...
static void rx_dhcp_server(struct mip_if *ifp, struct pkt *pkt) {
    ...
    if (op == 1 || op == 3) {          // DHCP Discover or DHCP Request
        ...
        tx_udp(ifp, ifp->ip, mg_htons(67), op == 1 ? ~0U : res.yiaddr, mg_htons(68), &res, sizeof(res));
    }
}
...
static size_t tx_tcp(struct mip_if *ifp, uint32_t dst_ip, uint8_t flags, uint16_t sport, uint16_t dport, uint32_t seq,
...
static size_t tx_tcp_pkt(struct mip_if *ifp, struct pkt *pkt, uint8_t flags, uint32_t seq, const void *buf,
                         size_t len) {
...
static struct mg_connection *accept_conn(struct mg_connection *lsn, struct pkt *pkt) {
    ...
    MG_DEBUG(("%lu accepted %I:%hu", c->id, 4, &c->rem.ip, mg_ntohs(c->rem.port)));
...
long mg_io_send(struct mg_connection *c, const void *buf, size_t len) {
    ...
    if (tx_tcp(ifp, c->rem.ip, TH_PUSH | TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq), mg_htonl(s->ack), buf,
               len) > 0) {
        s->seq += (uint32_t)len;
...
static void mip_poll(struct mip_if *ifp, uint64_t uptime_ms) {
    ...
    if (expired_1000ms && ifp->driver->up) {
        ...
        if (up != current) {
            ifp->state = up == false ? MIP_STATE_DOWN : ifp->enable_dhcp_client ? MIP_STATE_UP : MIP_STATE_READY;
    ...
    } else if (ifp->enable_dhcp_client == false && expired_1000ms && ifp->gw && arp_cache_find(ifp, ifp->gw) == NULL) {
    ...
    for (struct mg_connection *c = ifp->mgr->conns; c != NULL; c = c->next) {
        ...
        if (uptime_ms > s->timer) {
            if (s->ttype == MIP_TTYPE_ACK) {
                ...
                tx_tcp(ifp, c->rem.ip, TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq), mg_htonl(s->ack), "", 0);
            } else {
                MG_DEBUG(("%lu keepalive", c->id));
                tx_tcp(ifp, c->rem.ip, TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq - 1), mg_htonl(s->ack), "", 0);
                if (s->tmiss++ > 2)
...
void mg_connect_resolved(struct mg_connection *c) {
    ...
    MG_DEBUG(("%lu %I:%hu->%I:%hu", c->id, 4, &c->loc.ip, mg_ntohs(c->loc.port), 4, &c->rem.ip, mg_ntohs(c->rem.port)));
    mg_call(c, MG_EV_RESOLVE, NULL);
    if (c->is_udp) {
        mg_call(c, MG_EV_CONNECT, NULL);
    } else {
...
static void write_conn(struct mg_connection *c) {
    long len = c->is_tls ? mg_tls_send(c, c->send.buf, c->send.len) : mg_io_send(c, c->send.buf, c->send.len);
    if (len > 0) {
...
static void close_conn(struct mg_connection *c) {
    ...
    if (c->is_udp == false && c->is_listening == false) {   // For TCP conns,
        ...
        tx_tcp(ifp, c->rem.ip, TH_FIN | TH_ACK, c->loc.port, c->rem.port, mg_htonl(s->seq), mg_htonl(s->ack), NULL, 0);
    }
    mg_close_conn(c);
}
...
void qp_mark(unsigned int type, int len) {
    ...
    struct qpentry e = {.timestamp = (uint32_t)mg_millis(), .type = (uint16_t)type, .len = (uint16_t)len};
