...
int sound_start_play(char *device) {
    ...
    /*
    // This function call and the next have been replaced by the snd_pcm_hw_params_set_period_size_near() function call
    - N3SB December 2023
...
#endif
    /*
    // This function call and the previous have been replaced by the snd_pcm_hw_params_set_period_size_near() function
    call - N3SB December 2023 e = snd_pcm_hw_params_set_buffer_size_near(pcm_play_handle, hwparams, &n_frames); if (e <
    0) { fprintf(stderr, "*Error setting playback buffersize.\n"); return(-1);
            }
    */
...
int sound_loop() {
    ...
    #endif
      pcmreturn = snd_pcm_writei(loopback_play_handle, data_out, frames);		// Get a head start on filling the
    queue #if DEBUG > 0 printf("Loopback buffer filled with %d samples\n",pcmreturn); #endif
    */
    ...
    while (sound_thread_continue) {
...
#if DEBUG > 0
                printf(
                    "#### Partial Write ####"); // would signify that the playback channel didn't accept all the samples
#endif
            }
...
#if DEBUG > 0
                printf("Loop Counter: %d, Play PCM Write Error %d: %s  count = %d\n", loop_counter, pcmreturn,
                       snd_strerror(pcmreturn), pcm_play_write_error++);
#endif
                if (pcmreturn == -EPIPE) {
#if DEBUG > 0
                    printf("Samples Read: %d, Samples Written: %d, delta: %d, available %d\n", samples_read,
                           samples_written, samples_read - samples_written, pcm_write_avail);
...
#if DEBUG < 2
                // snd_pcm_recover(pcm_play_handle, pcmreturn, 1);		// Does not provide detailed error
                // message
#else
        ...
        while (ii < ret_card) {
            ...
            line_out[jj++] =
                output_i[ii] / LOOPBACK_LEVEL_DIVISOR; // Right Channel. Note: FLDIGI does not use the this channel.
        ...
        while (framesize > 0) {
            ...
            if (pcmreturn < 0) { // Handle an error condition from the snd_pcm_writei function
                ...
                printf("Loopback PCM Write %d bytes Error %d: %s  count = %d\n", framesize, pcmreturn,
                       snd_strerror(pcmreturn), pcm_loopback_write_error++);
#endif
