...
const static float hann(int const n, int const M) { return 0.5 - 0.5 * cos(2 * M_PI * n / (M - 1)); }
...
int window_filter(int const L, int const M, complex float *const response, float const beta) {
    ...
    fftwf_plan fwd_filter_plan =
        fftwf_plan_dft_1d(N, buffer, buffer, FFTW_FORWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
    fftwf_plan rev_filter_plan =
        fftwf_plan_dft_1d(N, buffer, buffer, FFTW_BACKWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
