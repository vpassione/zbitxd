...
///
static void synth_gfsk(const uint8_t *symbols, int n_sym, float f0, float symbol_bt, float symbol_period,
...
void ft8_poll(int seconds, int tx_is_on) {
    ...
    if ((ft8_tx1st == 1 && ((seconds >= 0 && seconds < 15) || (seconds >= 30 && seconds < 45))) ||
        (ft8_tx1st == 0 && ((seconds >= 15 && seconds < 30) || (seconds >= 45 && seconds < 59)))) {
        tx_on(TX_SOFT);
...
static char m1[32], m2[32], m3[32], m4[32], signal_strength[10], mygrid[10], reply_message[100];
