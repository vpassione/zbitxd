...
// Options for WISDOM_MODE from least to most rigorous are FFTW_ESTIMATE, FFTW_MEASURE, FFTW_PATIENT, and
// FFTW_EXHAUSTIVE The FFTW_ESTIMATE mode seems to make completely incorrect Wisdom plan choices sometimes, and is not
// recommended. Wisdom plans found in an existing Wisdom file will negate the need for time consuming Wisdom plan
// calculations if the Wisdom plans in the file were generated at the same or more rigorous level.
...
struct power_settings band_power[] = {
    {3500000, 4000000, 37, 0.002},    {5200000, 5800009, 40, 0.0015},   {7000000, 7300009, 40, 0.0015},
    {10000000, 10200000, 35, 0.0019}, {14000000, 14300000, 35, 0.0025}, {18000000, 18200000, 20, 0.0023},
    {21000000, 21450000, 20, 0.003},  {24800000, 25000000, 20, 0.0034}, {28000000, 29700000, 20, 0.0037}};
...
void fft_init() {
    ...
    plan_spectrum =
        fftw_plan_dft_1d(MAX_BINS, fft_in, fft_spectrum, FFTW_FORWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
...
struct rx *add_tx(int frequency, short mode, int bpf_low, int bpf_high) {
    ...
    r->plan_rev =
        fftw_plan_dft_1d(MAX_BINS, r->fft_freq, r->fft_time, FFTW_BACKWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
    fftw_export_wisdom_to_filename(wisdom_file);
...
void rx_linear(int32_t *input_rx, int32_t *input_mic, int32_t *output_speaker, int32_t *output_tx, int n_samples) {
    int i, j = 0;
...
void tx_process(int32_t *input_rx, int32_t *input_mic, int32_t *output_speaker, int32_t *output_tx, int n_samples) {
    int i;
...
*/
void sound_process(int32_t *input_rx, int32_t *input_mic, int32_t *output_speaker, int32_t *output_tx, int n_samples) {
    if (in_tx)
...
void set_rx_filter() {
    ...
    if (rx_list->mode == MODE_AM) {
        ...
        filter_tune(rx_list->filter, (1.0 * -rx_list->high_hz) / 96000.0, (1.0 * -rx_list->low_hz) / 96000.0, 5);
    else
        filter_tune(rx_list->filter, (1.0 * rx_list->low_hz) / 96000.0, (1.0 * rx_list->high_hz) / 96000.0, 5);
}
...
void sdr_request(char *request, char *response) {
    ...
    if (!strcmp(cmd, "stat:tx")) {
        ...
        if (rx_list->mode == MODE_AM) {
            filter_tune(tx_list->filter, (1.0 * 20000) / 96000.0, (1.0 * 30000) / 96000.0, 5);
            filter_tune(tx_filter, (1.0 * 20000) / 96000.0, (1.0 * 30000) / 96000.0, 5);
        } else if (rx_list->mode == MODE_LSB || rx_list->mode == MODE_CWR) {
            filter_tune(tx_list->filter, (1.0 * -3000) / 96000.0, (1.0 * -300) / 96000.0, 5);
            filter_tune(tx_filter, (1.0 * -3000) / 96000.0, (1.0 * -300) / 96000.0, 5);
        } else {
            filter_tune(tx_list->filter, (1.0 * 300) / 96000.0, (1.0 * 3000) / 96000.0, 5);
