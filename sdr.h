...
static inline float const cnrmf(const complex float x) { return crealf(x) * crealf(x) + cimagf(x) * cimagf(x); }
static inline double const cnrm(const complex double x) { return creal(x) * creal(x) + cimag(x) * cimag(x); }
